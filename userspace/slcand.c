...
void print_usage(char *prg)
{
	...
	fprintf(stderr, "         -b          (number of busses)\n");
	fprintf(stderr, "         -s <speed>  (set CAN speed 0..8, comma separated for multiple busses)\n");
	...
	if (speed && busses) {
        ...
        sprintf(nbuf, "C\r");
            for (int i = 0; i < busses; i++) {
                sprintf(&nbuf[2 + (i*5)], "B%dS%c\r", i, speed[i*2]);
            }
            printf("Sending: %s\n", nbuf);
            if (write(fd, nbuf, strlen(nbuf)) <= 0) {
                perror("write");
                exit(EXIT_FAILURE);
            }
            free(nbuf);
...
/*	if (btr) {
	...
	syslogger(LOG_NOTICE, "attached TTY %s to netdevices %s\n", ttypath, ifr.ifr_name);
